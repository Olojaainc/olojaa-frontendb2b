...
export default function ForgotPassword() {
    ...
    const redirectUrl = process.env.NEXT_PUBLIC_REDIRECT_URL || "http://localhost:3000/ChangePassword";

    const resetPassword = async (formValues:string) => {
        setLoading(true);
        setApiResponse(null);
       try{
            const res = await fetch('https://olojaa-backendb2b.onrender.com/api/v1/reset-password', {
                method: 'POST',
                headers: { 
            ...
            setApiResponse({ message: "Network error. Please check your internet connection." });
        }finally{
            setLoading(false);
        }
    }
    ...
    return (
        <AuthLayout>
            {apiResponse?.message && (
                <Alert className="fixed top-6 w-auto" message={apiResponse?.message} type={apiResponse.errors && !apiResponse.status ? "error" : "success"} showIcon closable />
            )}
            {apiResponse?.errors && (
                <div className="fixed top-6 w-auto">
                    {apiResponse.errors.email && (
                    <Alert message={apiResponse.errors.email } type="error" showIcon closable closeIcon />
                    )}
                    {apiResponse.errors.url && (
                    <Alert message={apiResponse.errors.url} type="error" showIcon closable closeIcon />
                    )}
                </div>
            )}
            <div className="bg-white w-[464px] h-auto px-8 py-10 rounded-3xl shadow-lg">
                ...
                <div className="flex flex-col">
                    ...
                    <button onClick={() => resetPassword(formValues.email)} className="bg-[var(--primary-400)] mb-4 text-white w-[400px] py-2.5 px-4 mt-4 rounded-xl"> {loading ? "Processing..." : "Send Reset Link"}</button>
